...
Use a hidden `bmemb-blocked-by` marker only when an issue must wait
...
Do not use `bmemb-blocked-by` to group children under the roadmap
...
- no `bmemb-roadmap-id` marker
- no `bmemb-blocked-by` marker
...
- `## Tracks`
- `## Success criteria`
- one `<!-- bmemb-roadmap-id: <roadmap-id> -->` marker
...
- `## Candidate files`
- `Blocked by #NNN`
- `<!-- bmemb-blocked-by: <roadmap-id> -->` when a separate roadmap
  must close first
- `## Notes`
...
- each `bmemb-blocked-by` marker points to a real roadmap and is
